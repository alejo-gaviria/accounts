...
currency + initial_balance, and rejects a negative initial_balance via
the same InvalidAmount domain error credit/debit already use.
"""
...
@pytest.mark.asyncio
async def test_creates_account_with_default_currency_and_zero_balance():
...
@pytest.mark.asyncio
async def test_creates_account_with_given_currency_and_initial_balance():
    accounts = FakeAccountRepository({})
    use_case = CreateDummyAccountUseCase(FakeUnitOfWork(accounts, FakeLedgerRepository()))

    account = await use_case.execute(currency="EUR", initial_balance=Decimal("50.00"))
